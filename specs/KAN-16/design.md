...
- **Inngest** — voir `tech/setup.md` § Inngest. Statut *Partiel* (2026-05-16) : compte + clés Production OK côté Vercel + `.env.local`. Reste à scaffolder `packages/jobs/` + endpoint `/api/v1/inngest` + sync de l'app côté dashboard, à faire à l'implémentation. **Cette US devient le premier consommateur d'Inngest du repo** — décision à acter au journal §18 d'ARCHITECTURE.md.
