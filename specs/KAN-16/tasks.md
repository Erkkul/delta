...
- [ ] **Setup API Sirene INSEE** (nouvelle entrée `tech/setup.md` § Externe) : créer compte API INSEE, récupérer token bearer, ajouter `INSEE_SIRENE_TOKEN` dans Vercel + `.env.local.example`, documenter
...
- [ ] Créer le client Sirene dans `packages/core/src/producer/insee-client.ts` (fetch avec cache token 23 h, mapping réponse → `{ verified, reason? }`)
...
- [ ] Ajouter dans `apps/web/.env.local.example` : `STRIPE_WEBHOOK_SECRET_LOCAL`, `INSEE_SIRENE_TOKEN`, `INNGEST_EVENT_KEY`, `INNGEST_SIGNING_KEY` (sans valeurs)
