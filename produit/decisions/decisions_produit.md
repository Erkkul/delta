...
- **2026-05-13** — **Splash écran d'entrée du flow auth** sur route `/welcome`. Hero photo terroir + CTAs « Créer un compte gratuit » / « J'ai déjà un compte » + mention RGPD légère en bas. La landing publique `/` (existante) reste indépendante. Conforme à la maquette `*-01-authentication.html` (écran AU-01).
- **2026-05-13** — **Sélection multi-rôle au signup, sur écran dédié post-vérif email**. Précise et confirme la décision 2026-05-03 « multi-rôles progressif ». Le compte créé par le signup démarre avec **zéro rôle** ; l'utilisateur sélectionne 1-3 rôles (rameneur / producteur / acheteur) à l'écran AU-06 « Choix rôle » du flow auth, en multi-select. Bandeau d'info « Vous pouvez cumuler les rôles, par exemple un producteur peut aussi être acheteur ou rameneur ». Supersede la mention « rôle initial à l'inscription » du `specs/KAN-2/proposal.md` v1.
- **2026-05-13** — **Vérification email obligatoire au signup** (OTP 6 chiffres). Écran AU-04 du flow auth : « Vérifiez votre email — Nous avons envoyé un code à 6 chiffres à \<email\> ». Bouton « Renvoyer le code » + lien « Modifier l'email ». Actif dès le MVP. Cohérent avec la config Supabase Auth déjà en place (`Confirm email ON`, cf. `tech/setup.md`). [SUPERSEDED — voir entrée *Pas de double opt-in mail au MVP* du `proposal.md` KAN-2 v1, retirée.]
- **2026-05-13** — **Consents CGU/RGPD implicites au signup**. Mention « En continuant, vous acceptez nos CGU et politique de confidentialité » au bas de l'écran AU-01 (Splash) et AU-02 (Signup), plutôt que checkboxes obligatoires séparées. La version des consents (`termsVersion`, `privacyVersion`, `acceptedAt`) est tracée automatiquement en DB (`users.metadata.consents`) à la création de compte par le route handler `/api/v1/auth/signup` et par le callback OAuth. Compromis UX/conversion vs RGPD strict — la traçabilité serveur reste forte.
- **2026-05-13** — **Politique de mot de passe : 10 caractères minimum, 1 majuscule, 1 minuscule, 1 chiffre**. Confirme la config Supabase Auth (`tech/setup.md` 2026-05-08). Le hint affiché à l'utilisateur sur l'écran AU-02 est synchronisé avec cette politique (la maquette mentionnait « 8 caractères », elle sera ajustée). Strength meter visuel maintenu côté UI.
...
- **2026-05-01** — **Catalogue acheteur (AC-04) filtré aux produits matchables** uniquement. Un produit n'apparaît que si au moins un rameneur a fait des trajets compatibles récents (seuil exact à régler côté dev, proposition ≥ 2 trajets sur 30 j ou ≥ 1 trajet actif). Évite la déception "j'ai mis ça en wishlist depuis 6 mois". Producteurs hors couverture rameneurs restent invisibles tant que la demande n'attire pas un rameneur. *(Filtre de matchabilité différé à l'implémentation — voir entrée 2026-06-23.)*
- **2026-06-23** — **Catalogue acheteur livré au MVP sans filtre de matchabilité par trajet** (KAN-28). La couche trajets (KAN-41) et le pipeline opportunités (KAN-42) n'existant pas encore, le catalogue (AC-03/04/05) affiche **tous les produits publiquement visibles** (produit `active` d'un producteur vérifié + payouts ON + non en pause, fenêtre de disponibilité couvrante — prédicat RLS `products_select_public`). Les badges trajet, la section « Disponible cette semaine » et les filtres zone/disponibilité/trajet sont rendus en état différé/inerte. **L'intention cible (filtre matchable ≥ 2 trajets/30 j ou ≥ 1 trajet actif) reste valable** et sera câblée par KAN-42 — la décision 2026-05-01 n'est pas annulée, seulement différée. Dette temporaire assumée plutôt qu'un catalogue vide en attendant la couche matching. Cohérent avec la posture KAN-19/27 (livrer l'enveloppe réelle, différer ce qui dépend de tables absentes).
