...
- **Code** : KAN-2 (création de compte) livré sur main. KAN-3 (connexion) en cours sur `claude/kan-3-flow-connexion` — page `/login`, endpoints `/api/v1/auth/login` et `/api/v1/auth/logout`, helper rate-limit Upstash dans `packages/core/rate-limit`. Reste à attaquer : KAN-157 (mot de passe oublié), puis onboardings par rôle (KAN-16 / KAN-25 / KAN-37).
...
- **Branch protection `main`** : la règle `Settings → Branches → main` garde « Require status checks to pass before merging » (avec le check `Lint, typecheck, build` obligatoire), mais **« Require branches to be up to date before merging » est désactivée** (décision 2026-05-19 après KAN-23). Raison : avec cette option active, dès qu'une PR mapping ou une autre PR merge entre temps, la branche de la PR en cours devient out-of-date et l'auto-merge se bloque jusqu'à un `Update branch` manuel. La CI tournant déjà sur la PR et GitHub re-validant le merge commit final, le risque d'introduire un état non testé sur main est négligeable pour un repo séquentiel comme Delta. Conséquence : l'auto-merge devient vraiment hands-off — pas besoin d'`update_pull_request_branch` côté agent.
