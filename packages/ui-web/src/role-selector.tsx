...
/**
 * RoleSelector AU-06 — multi-select 1..3 rôles. Bandeau « Email vérifié »
 * en haut, 3 RoleCards cliquables, CTA Continuer désactivé tant qu'aucun
 * rôle n'est sélectionné. Bandeau info en bas rappelant le cumul possible.
 */
  ...
  return (
    <div className="flex flex-col gap-6">
      <header className="flex flex-col gap-3">
        <span className="inline-flex items-center gap-1.5 self-start rounded-pill bg-green-50 px-3 py-1 font-body text-xs font-bold uppercase tracking-[0.04em] text-green-800">
          <CheckIcon />
          Email vérifié
        </span>
        ...
        {ROLES.map((role) => {
          ...
          return (
            <button
              ...
              className={[
                "flex items-start gap-4 rounded-lg border bg-white p-4 text-left transition focus:outline-none focus:shadow-focus",
                selected
                  ? "border-green-600 shadow-active"
                  : "border-cream-300 hover:border-cream-400 hover:shadow-subtle",
              ].join(" ")}
            >
              <span
                className={[
              ...
              <span className="flex flex-1 flex-col gap-1">
                <span className="flex items-center justify-between gap-3">
                  <span className="font-display text-lg font-semibold text-cream-950">
                    {copy.title}
                  </span>
                  <span
                    className={[
      ...
      <div className="flex items-start gap-2 rounded-md bg-cream-100 p-3 font-body text-xs leading-relaxed text-cream-700">
        <InfoIcon />
        <span>
...
function InfoIcon() {
  return (
    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true" style={{ flexShrink: 0, marginTop: 1, color: "#A8A29E" }}>
